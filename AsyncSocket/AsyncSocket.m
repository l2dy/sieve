...
@interface AsyncReadPacket : NSObject
{
  @public
	...
	long tag;
    ReadCompletionBlock_t completionBlock;
}
- (id)initWithData:(NSMutableData *)d
       startOffset:(CFIndex)s
        ...
        terminator:(NSData *)e
               tag:(long)i
             block: (ReadCompletionBlock_t)block;
...
@interface AsyncWritePacket : NSObject
{
  @public
	...
	long tag;
	NSTimeInterval timeout;
    WriteCompletionBlock_t completionBlock;
}
- (id)initWithData:(NSData *)d timeout:(NSTimeInterval)t tag:(long)i block: (WriteCompletionBlock_t) block;
@end
...
- (void)readDataWithTimeout:(NSTimeInterval)timeout
               ...
               bufferOffset:(NSUInteger)offset
                  maxLength:(CFIndex)length
                        tag:(long)tag
{
	[self readDataWithTimeout:timeout buffer:buffer bufferOffset:offset maxLength:length tag:tag block: nil];
}
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag block: (ReadCompletionBlock_t) block;
{
	[self readDataToLength:length withTimeout:timeout buffer:nil bufferOffset:0 tag:tag block: block];
}
...
- (void)readDataToLength:(CFIndex)length
            ...
            bufferOffset:(NSUInteger)offset
                     tag:(long)tag
{
	[self readDataToLength:length withTimeout:timeout buffer:nil bufferOffset:offset tag:tag block: nil];
}
...
- (void)readDataToData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag block: (ReadCompletionBlock_t) block;
{
	[self readDataToData:data withTimeout:timeout buffer:nil bufferOffset:0 maxLength:-1 tag:tag block: block];    
}
...
- (void)readDataToData:(NSData *)data
          ...
          bufferOffset:(NSUInteger)offset
             maxLength:(CFIndex)length
                   tag:(long)tag
{
	[self readDataToData:data withTimeout:timeout buffer:nil bufferOffset:0 maxLength:length tag:tag block: nil];
}
...
- (void)completeCurrentRead
{
    ...
    if (theCurrentRead->completionBlock) theCurrentRead->completionBlock( result );
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag block: (WriteCompletionBlock_t) block;
{
	if (data == nil || [data length] == 0) return;
	if (theFlags & kForbidReadsWrites) return;
	
	AsyncWritePacket *packet = [[AsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag block: block];
...
{
    [self writeData:data withTimeout:timeout tag:tag block: nil];
}
...
- (void)completeCurrentWrite
{
    ...
    if (theCurrentWrite->completionBlock) theCurrentWrite->completionBlock();
